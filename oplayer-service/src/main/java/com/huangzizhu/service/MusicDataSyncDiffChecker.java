...
import com.huangzizhu.service.InvalidMusicMarker;
import com.huangzizhu.utils.MusicFeatureReader;
import com.huangzizhu.service.NewMusicSubmitter;
...
public class MusicDataSyncDiffChecker implements Runnable{
    ...
    public void start() {
        ...
        getLocalData();
        //先用本地数据作为标准，找出数据库的不同
        DBMusicMD5.stream().filter(md5 -> !localMusic.contains(md5)).forEach(MD5->invalidMusicMarker.submitMD5(MD5));
        //在对比数据库找到本地新增
        localMusic.stream().filter(md5 -> !DBMusicMD5.contains(md5)).forEach(musicFileFeature ->newMusicSubmitter.submitMusic(musicFileFeature));
        log.info("数据差异化比较完成");
    }
